    ...
    <div className="min-h-screen bg-dorren-black text-white font-sans selection:bg-dorren-lightBlue selection:text-dorren-black pb-32">
      ...
      >
        <div className="space-y-8">
          ...
          <form className="bg-[#080808] p-8 border-l-2 border-dorren-darkBlue/30 relative overflow-hidden transition-colors">
             <div className="absolute top-0 right-0 w-px h-full bg-gradient-to-b from-transparent via-dorren-darkBlue/20 to-transparent"></div>
             <h4 className="text-xs uppercase tracking-[0.25em] text-white/40 mb-8 flex items-center gap-4">
               <span className="w-8 h-px bg-dorren-lightBlue/50"></span>
               Данные проекта
             </h4>
             
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                <div className="space-y-2 group">
                  <label className="block text-[10px] uppercase tracking-[0.2em] text-white/40 font-medium group-focus-within:text-dorren-lightBlue transition-colors">
                    Менеджер
                  </label>
                  <input 
                    type="text" 
                    value={managerName}
                    onChange={(e) => setManagerName(e.target.value)}
                    className="w-full bg-black border border-white/10 p-3 text-sm text-white placeholder:text-white/10 focus:border-dorren-lightBlue focus:bg-dorren-darkBlue/10 outline-none transition-all duration-300"
                ...
                <div className="col-span-1 md:col-span-2 space-y-2 group">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-black border border-white/10 p-3 text-sm text-white placeholder:text-white/10 focus:border-dorren-lightBlue focus:bg-dorren-darkBlue/10 outline-none transition-all duration-300 min-h-[100px] resize-y"
                    placeholder="Дополнительная информация о заказе..." 
                  />
                </div>

                <div className="col-span-1 md:col-span-2 mt-6 flex gap-4 pt-4 border-t border-white/5">
                  <Button 
                     type="button"
                     className={`flex-1 transition-all duration-300 ${saveSuccess ? 'bg-green-500/20 text-green-400 border border-green-500/50' : 'bg-dorren-darkBlue text-white hover:bg-dorren-lightBlue hover:text-dorren-black'}`}
                  ...
                  <Button 
                    className={`flex-1 transition-all duration-300 ${pdfSuccess ? 'bg-green-500/20 text-green-400 hover:bg-green-500/30 border border-green-500/50' : ''}`}
                ...
                {(pdfSuccess || saveSuccess || errorMsg) && (
                    <div className="col-span-1 md:col-span-2">
                        {(pdfSuccess || saveSuccess) && (
                            <p className="text-center text-[10px] text-white/30 mt-2 uppercase tracking-widest animate-in fade-in">
                            Операция выполнена успешно
                            </p>
                        )}
                        {errorMsg && (
                            <p className="text-center text-xs text-red-400 mt-2 animate-in fade-in bg-red-500/10 p-2 border border-red-500/20 rounded">
                            {errorMsg}
                            </p>
                        )}
                    </div>
